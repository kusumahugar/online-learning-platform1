      ...
      <header className="border-b border-slate-800 bg-slate-900/90 backdrop-blur-sm">
        <div className="container flex flex-wrap items-center justify-between gap-4 py-4">
          ...
          <nav className="flex flex-wrap items-center gap-3">
            ...
            {!user && <Link to="/register" className="hover:text-white">Register</Link>}
            {user && (
              <button onClick={logout} className="rounded bg-cyan-500 px-4 py-2 text-sm font-medium text-slate-950 hover:bg-cyan-400">
                Logout
              </button>
            )}
          </nav>
